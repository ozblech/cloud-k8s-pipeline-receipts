...
resource "aws_iam_policy" "minikube_s3_policy" {
  name = "minikube-s3-policy"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [{
      Effect   = "Allow",
...
resource "aws_iam_role_policy_attachment" "minikube_s3_attach" {
  role       = aws_iam_role.minikube_role.name
  policy_arn = aws_iam_policy.minikube_s3_policy.arn
}
